...
if __name__ == "__main__":
    ...
    experiment_result_path = "/share/tyz/data_experiment/DCASE_2022_few_shot/distill_extractor_9_1"
    parser.add_argument('-pred_file', type=str, help='csv predictions file',default=os.path.join(experiment_result_path,"basic.csv"))
    parser.add_argument('-ref_files_path', type=str, help='path to the ground truth csvs folder',default="/share/database/tyz/Development_Set/Validation_Set")
    ...
    parser.add_argument('-val_path', type=str, help='path to validation folder with wav and csv files',
                        default="/share/database/tyz/Development_Set/Validation_Set")
    parser.add_argument('-adaptive_evaluation_file', type=str, help="name of prost processed prediction file to be saved",
                        default=os.path.join(experiment_result_path, "adaptive_basic.csv"))
    parser.add_argument('-fixed_evaluation_file', type=str, help="name of prost processed prediction file to be saved",
                        default=os.path.join(experiment_result_path, "fixed_basic.csv"))
    args = parser.parse_args()
    # print(args)
    post_proc.post_processing(args.val_path, args.pred_file, args.adaptive_evaluation_file)
    post_proc_new.post_processing(args.val_path, args.pred_file, args.fixed_evaluation_file)
    original = evaluate(args.pred_file, args.ref_files_path, args.team_name, args.dataset, args.savepath)
    adaptive = evaluate(args.adaptive_evaluation_file, args.ref_files_path, args.team_name, args.dataset, args.savepath)
    fixed = evaluate(args.fixed_evaluation_file, args.ref_files_path, args.team_name, args.dataset, args.savepath)
    print("=================== original ============================")
    print(original)
    print("=================== adaptive ============================")
    print(adaptive)
