...
def evaluate(pred_file_path, ref_file_path, team_name, dataset, savepath, metadata=[]):
    ...
    #read prediction csv
    pred_csv = pd.read_csv(pred_file_path, dtype=str,engine='python')
    #verify headers:
    ...
    for audiofilename in list(pred_events_by_audiofile.keys()):
        ...
        ref_events_this_audiofile_all = ref_events_this_audiofile_all.sort_values(by="Starttime",ascending=True)
    ...
    for audiofilename in list_all_audiofiles:
        if audiofilename not in counts_per_audiofile.keys():
            ...
            ref_events_this_audiofile = ref_events_this_audiofile.sort_values(by="Starttime", ascending=True)
...
if __name__ == "__main__":
    ...
    def test(conf: DictConfig):
        parser = argparse.ArgumentParser()
        experiment_result_path,val_path = conf.eval.eval_model, conf.path.eval_dir
        ...
        parser.add_argument('-team_name', type=str, help='team identification', default="Yizhou_Tan")
        parser.add_argument('-dataset', type=str, help="which set to evaluate: EVAL or VAL", default="VAL")
        parser.add_argument('-savepath', type=str, help="path where to save the report to", default=experiment_result_path)
        ...
        skip = 1
        for iteration in range(begin,end,skip):
            print("Iteration: {}".format(iteration))
            # print(args)
            pred_file = os.path.join(experiment_result_path,"iteration_{}.csv".format(iteration))
            adaptive_evaluation_file = os.path.join(experiment_result_path, "adaptive_iteration_{}.csv".format(iteration))
            fixed_evaluation_file = os.path.join(experiment_result_path, "fixed_iteration_{}.csv".format(iteration))
            post_proc.post_processing(args.val_path, pred_file, adaptive_evaluation_file)
            post_proc_new.post_processing(args.val_path, pred_file, fixed_evaluation_file)
            original.append(evaluate(pred_file, args.ref_files_path, args.team_name, args.dataset, args.savepath))
            adaptive.append(evaluate(adaptive_evaluation_file, args.ref_files_path, args.team_name, args.dataset, args.savepath))
            fixed.append(evaluate(fixed_evaluation_file, args.ref_files_path, args.team_name, args.dataset, args.savepath))

        print("=================== original ============================")
        index = 0
        add = "/home/tyz/dcase_task5/fmeasure_by_iteration.json"
        ...
        data[name] = []
        for iteration in range(begin,end,skip):
            print("Iteration_{}  {}".format(iteration,original[index]))
            data[name].append(original[index]['fmeasure (percentage)'])
            index += 1
        with open(add, "w") as f:
            json.dump(data,f)
        print(data)
        ...
        # index = 0
        # for iteration in range(begin,end,skip):
        #     print("Iteration_{}  {}".format(iteration,adaptive[index]))
        #     index += 1
        # print("=================== fixed ============================")
        # index = 0
        # for iteration in range(begin,end,skip):
        #     print("Iteration_{}  {}".format(iteration,fixed[index]))
        #     index += 1
